...
# device to run the network on and import the content and style images
# (will choose GPU if available)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# >> load the images >>
# size of output image
imsize = 512 if torch.cuda.is_available() else 128  # if no GPU
...
style_img = image_loader('images/monetclaude.jpg')
content_img = image_loader('images/content.png')
...
def imshow(tensor, title=None):
    image = tensor.cpu().clone()  # close tensor
...
class ContentLoss(nn.Module):
    ...
    def __init__(self, target, ):
        ...
        # detach the target content from the tree used to
        # dynamically compute the gradient: stated value, not a variable.
        # other forward method of criterion will throw error
...
def gram_matrix(input):
    ...
    # normalize values of gram matrix by dividing number of element
    # in each feature maps
...
# >> importing pre-trained neural network (19 layer VGG network)
...
# create module to normalize input image to easily put it in nn.Sequential
...
def get_style_model_and_losses(cnn, normalization_mean, normalization_std, style_img,
    ...
    # assuming that cnn is a nn.Sequential, make a new nn.Sequential to put in
...
# select input image
input_img = content_img.clone()  # copy of content image
# if you want to use white noise instead uncomment the below line:
# input_img = torch.randn(content_img.data.size(), device=device)

# add original input image to figure:
plt.figure()
...
def get_input_optimizer(input_img):
    # this line shows that input is a parameter that requires gradient
...
def run_style_transfer(cnn, normalization_mean, normalization_std,
    ...
    # optimize the input and not the model parameters
